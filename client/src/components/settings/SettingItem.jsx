...
function ToggleItem({setting}){
    ...
    return(
        <div className="formControl">
            ...
            <div className="ml-10">
                <input id={setting.name} name={setting.name} type="checkbox"  onClick={onChange} value={value} defaultChecked={value === 'on' ? true : false} className="toggle toggle-info"  />
            </div>                    
        </div>
    )
}
    ...
    return (
        <div className="formControl">
            ...
            <div className="ml-10 flex">
                <input name={setting.name} id={setting.name} type="range" min={0} max="100" className="range range-info" value={value} onChange={onChange} />
...
function TextAreaItem({setting}){
    ...
    return(
        ...
        <div className="ml-10 block">
                <textarea id={setting.name} name={setting.name} rows={value.length} value={ formatTextArea() } onChange={onChange}  className="textarea textarea-bordered textarea-info w-full max-w-xs" />
        </div>
    </div> 
    )
}
...
function TextItem({setting}){
    ...
    return(
        <div className="formControl">
            ...
            <div className="ml-10 block">
                    <input type="text" name={setting.name} value={value} onChange={onChange}  className="input input-bordered input-info w-full max-w-xs" />
            </div>
        </div>
    )
}
            ...
            <div className="ml-10" >
                {  setting.options.map((option, index) => {
                    ...
                    <label key={index} className="cursor-pointer">
                        <input key={index} className="radio radio-info" name={setting.name} id={setting.name} type="radio" defaultChecked={option.value === value? true:false} value={option.value} onChange={onChange}/>
